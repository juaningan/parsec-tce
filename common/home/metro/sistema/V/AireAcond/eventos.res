!tabla que se carga cuando se pincha en Eventos

titles: title
*.title.fontList: \
	          -b&h-lucida-bold-r-normal-sans-18-180-75-75-p-120-iso8859-1
*.title.labelString: @R@EVENTOS AIRE ACONDICIONADO@P@
*.title.foreground:         black
*.title.xrtLblBorderType:   BORDER_NONE
*.title.xrtTblLocation:     LOCATION_TOP
*.title.xrtTblTopOffset:    0
*.title.xrtTblBottomOffset: 5

! Table
*.background:                  gray80
!*.xrtTblCellValues:            eventos.ascii
!*.xrtTblFontListSeries: (all all -dt-application-bold-r-normal-serif-14-100-100-100-m-90-iso8859-1)
*.xrtTblFontListSeries: (all all -adobe-courier-medium-r-normal--12-120-75-75-m-70-iso8859-1)
*.xrtTblBackgroundSeries:      (all all #FFE7CD)(all 0 gray80)(all 58-59 gray80) 
*.xrtTblForegroundSeries:      (all all Midnight Blue)
*.xrtTblPixelWidthSeries:      (all all 15)
*.xrtTblPixelHeightSeries:     (all all 22)(49 all 25)
*.xrtTblBorderTypeSeries:      (all all border_etched_in) 
*.xrtTblShadowThickness:       3
*.xrtTblShadowType:            shadow_out
*.xrtTblFrameShadowType:  border_out
*.xrtTblMarginWidth:           0
*.xrtTblMarginHeight:          0
*.xrtTblNumRows:               50
*.xrtTblNumColumns:            60
*.xrtTblAlignmentSeries:       (ALL ALL ALIGNMENT_MIDDLEBEGIN)
*.xrtTblEditableSeries:	       (ALL ALL False)
*.xrtTblTraversableSeries:     (ALL ALL False)
*.xrtTblAllowCellResize:       RESIZE_NONE
*.xrtTblSelectionPolicy:       SELECT_NONE
*.xrtTblBorderSidesSeries:     (ALL ALL BORDERSIDE_NONE)
!*.xrtTblBorderSidesSeries:     (ALL ALL BORDERSIDE_ALL)
*.xrtTblTrackCursor:           False
*.xrtTblDisplayClipArrows:     False
*.xrtTblDisplayHorizScrollBar:     DISPSB_NEVER

